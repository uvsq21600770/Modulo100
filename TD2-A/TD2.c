...
double simul (double p0, double p1, double p2)
{
    ...
    unsigned long int DS = 10000; // La longueur du manchon
    double h = 1e-4; // La hauteur relative du manchon
    double m = 0.0; // Le milieu du manchon la valeur initiale est pas vraiment importante
...
fprintf(F, "%lu %lu %lf\n", T, N, S/(T+1));
    ...
    fclose(F);
#endif
if(T < TMAX) printf("ok "); else printf("PB ");
printf("%lu %lf\n", T, S/(T+1));
    return S/(T+1);
}
