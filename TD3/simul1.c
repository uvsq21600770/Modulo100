...
void simul_MM1 (double lambda, double mu) {
	...
	unsigned long int nb_arr = 0;
	unsigned long int nb_serv = 0;
...
if (nb_event % 1000000==0) {
	printf("%.3le %lu %.3le %.3le %.3le\r",T,N,(double)nb_event,(double)nb_arr,(double)nb_serv);
	fflush(stdout);
	}
#endif
	}
